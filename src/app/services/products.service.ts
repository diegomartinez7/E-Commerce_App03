...
export class ProductsService {
  ...
  public getProductos(filtro) {
    const ArrayFiltrado: object[] = [];
    for(let producto of this.Productos){
      if(producto.department==filtro)
  ...
  public muestra(){
    const ArrayTrinity: object[] = [];
    let i = 0;
    for(let producto of this.Productos){
      if(i==3)
        break;
      ArrayTrinity.push(producto);
      i++;
    }
    return ArrayTrinity;
  }

  public buscarProducto(termino) {
    const ArrayResultados: object[] = [];
    termino = termino.toLowerCase();
    for(let producto of this.Productos){
      if(producto.brand.toLowerCase().indexOf(termino) >= 0 || producto.model.toLowerCase().indexOf(termino) >= 0){
